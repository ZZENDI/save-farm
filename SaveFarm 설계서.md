# 농작물 지킴이
<h1 style='background-color: rgba(55, 55, 55, 0.4); text-align: center'>농산물 지킴이 API 설계(명세)서</h1>

해당 API 명세서는 '농산물지킴이 - SaveFarm'의 REST API를 명세하고 있습니다.  
...
SaveFarm 서비스의 인증 및 인가와 관련된 REST API 모듈입니다.  
...
```bash
curl -v -X POST "http://localhost:8000/api/v1/auth/sign-in" \
 -d "userId=qwer1234" \
 -d "password=P!ssw0rd"
```
...
```bash
curl -v -X POST "http://localhost:8000/api/v1/auth/id-check" \
 -d "userId=qwer1234"
```
...
```bash
curl -v -X POST "http://localhost:8000/api/v1/auth/tel-auth" \
...
```bash
curl -v -X POST "http://localhost:8000/api/v1/auth/tel-auth-check" \
 -d "telNumber=01011112222" \
 -d "authNumber=1234"
```
...
```bash
curl -v -X POST "http://localhost:8000/api/v1/auth/sign-up" \
 -d "name=김도연"\
 -d "userId=qwer1234"\
 -d "password=qwer1234"\
 -d "telNumber=01011112222"\
